package com.estebi.fogo1.ui.user.adapter
...
class MyPostsAdapter : RecyclerView.Adapter<MyPostsAdapter.PostsHolder>() {
    ...
    inner class PostsHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindView(posts: Posts) {
            ...
            val myPostImg = itemView.findViewById<ImageView>(R.id.idIVCourse)
            val img = "${posts.userImgPosts}";
            Picasso.get().load(img).into(myPostImg)
